...
int main(int argc, char* argv[]) {
    ...
    if (mode == "1") {
        ...
        std::stringstream ss(headerLine.substr(1));
        std::string item;
        while (std::getline(ss, item, ';')) {
        ...
        while (getline(input, line)) {
            ...
            while (line.find(';') != std::string::npos) {
                ...
                bool found = false;
                for (const auto& uniqueTransition : uniqueTransitions) {
                    if (uniqueTransition.state == transition.state && uniqueTransition.output == transition.output) {
                        found = true;
                        break;
                    }
                }
        ...
        for (size_t i = 0; i < inputs.size(); ++i) {
            ...
            for (const auto& uniqueTransition : uniqueTransitions) {
                ...
                for (size_t j = 0; j < uniqueTransitions.size(); ++j) {
                    if (uniqueTransitions[j].state == tempTransition.state && uniqueTransitions[j].output == tempTransition.output) {
                        transitionIndex = j;
                        break;
                    }
                }
        ...
        std::string line;
        getline(input, line);
        std::vector<std::string> outputs = ParseCSVLine(line.substr(1));

        getline(input, line);
        output << line << "\n";
        std::vector<std::string> states = ParseCSVLine(line.substr(1));
        ...
        while (getline(input, line)) {
            ...
            std::vector<std::string> transitions = ParseCSVLine(line);
            for (const auto& state : transitions) {
                output << state << "/" << stateOutputMap[state] << ";";
            }
            output << "\n";
